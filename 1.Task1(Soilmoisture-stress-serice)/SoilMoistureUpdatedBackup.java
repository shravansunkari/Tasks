...
public class SoilMoistureStressService implements ISoilMoistureStressService {
	...
	public Map<Long, Double> getAllGridRFStressData(int modelDate) throws DSPException, ObjectNotFoundException {
		...
		for (ISoilMoistureNRSCGridData smNRSCGridData : smNRSCGridDataList) {
			...
			/*
			 * else{
			 * System.out.println("No Villages for grid"+smNRSCGridData.
			 * getGridId());
			 * }
			 */
		}
	...
	public Map<String, ISoilMoistureStressData> getAllSMStressData(int modelDate, double rootZoneDepth)
		 ...
		 * level average]
		 * Iterate over each village under the Mandal [and compute Mandal level
		 * average]
		 * 
		...
		for (String districtName : allDistToMandMap.keySet()) {
			...
			for (String mandalName : mandalList) {
				 ...
				 * is available,
				 * This total will be used to compute the average by dividing
				 * with the summed up values
				 * 
				 */
	...
	protected Map<String, ISMRainfallStressData> getRFStressDataForVillages(int modelDate)
		 ...
		 * else
		 * modelDate timeStamp must be greater than 8 : 30 i.e rainfall day
		 * start time
		 */
		 ...
		 * Step 2 : iterate over the list of rainfall station UUIDs under that
		 * village
		 * and compute average over all the stations under the village to get
		 * the village level data
		 * 
		 */
	...
	public Map<String, Map<Integer, ISoilMoistureStressData>> getSMStressDataForRange(int startDate, int endDate,
		...
		System.out.println("\n\nTime lag : Map(733) time lag  t1=" + t1 + " C= " + System.currentTimeMillis() + "===> "
			...
			for (String villageName : villageList) {
				...
				for (Integer modelDate : villDateToDataMap.keySet()) {
					...
					if (mandStressData == null) {
						 ...
						 * available,
						 * This total will be used to compute the average by
						 * dividing with
						 * the summed up values
						 * 
						 */
		...
		System.out.println("\n\nTime lag : Map(817) time lag  t1=" + t1 + " C= " + System.currentTimeMillis() + "===> "
				+ (System.currentTimeMillis() - t1));
		...
		System.out.println("\n\nTime lag : Map(833) time lag  t1=" + t1 + " C= " + System.currentTimeMillis() + "===> "
				+ (System.currentTimeMillis() - t1));
		return resultMap;
	}
	...
	public Map<String, Map<Integer, Double>> getSMStressRFDataForDate(int startDate, int endDate,
		...
		System.out.println("\n\nTime lag : Map(887) time lag  t1=" + t1 + " C= " + System.currentTimeMillis() + "===> "
				+ (System.currentTimeMillis() - t1));
		/*
		 ...
		 * 6. Find out it's date and compute sum with the existing data in M2
		 * for that date and store it back in M2
		 * 7. Increment the dataRecordsCount for that date in M1
		 * 8. For each (date, Data) pair in M2
		 * 9. compute average by dividing the Data with the 'dataRecordsCount'
		 * avilable from M1 for that date.
		 * 
		 */
		t1 = System.currentTimeMillis();
		for (String villageName : villageSMLocDataMap.keySet()) {
			...
			for (ISMLocMapData gridData : gridsUnderVillage) {
				...
				/*
				 * Sum all the station grid values in a variable
				 * Store the count of stations in a variable
				 */
				double value = 0.0;// to store sum of all the station grids under a village
				int count = 0;// to store total count of stations
				int modelDate = 0;

				for (IWaterQuantityEventData wqEventData : rfLocDataList) {
					modelDate = DateUtils.getModelDateFromTs(wqEventData.getEventGenTs());
					if (wqEventData.getLevelValue1() >= 0) {
						value = value + wqEventData.getLevelValue1();
						count++;
					}
				}
				// put in map here
				dateToLevelMap.put(modelDate, value);
				dateToGridCount.put(modelDate, count);

				if (dateToLevelMap.size() == 0) {
					if (IS_DEBUG_ENABLED) {
						System.out.println(
								"ZBXXX : SMStressService - getSMStressRFDataForDate, No rainfall data available "
										+ "for village = " + villageName + ", between " + startDate + " and "
										+ endDate);
					}
					continue;
				}
				...
				for (Integer modelDate1 : dateToLevelMap.keySet()) {
					if (dateToGridCount.get(modelDate1) != 0) {
						dateToLevelMap.put(modelDate1,dateToLevelMap.get(modelDate1) / dateToGridCount.get(modelDate1));
					} else {
						dateToLevelMap.put(modelDate1, DSPConstants.NO_DATA);
					}
				}
		...
		System.out.println("\n\nTime lag : Map(993) Village Level time lag  t1=" + t1 + " C= "
		...
		for (String mandalName : mandalVillMap.keySet()) {
			...
			for (String villageName : villageList) {
				 ...
				 * Store the count of stations in a variable
				 */
				double value = 0.0;// to store sum of all the villages under
									// madal
				int count = 0;// to store count of all villages under madal
				int model_date = 0;
				for (Integer modelDate : villDateToLevelMap.keySet()) {
					model_date = modelDate;
					if (villDateToLevelMap.get(modelDate) >= 0) {
						value = value + villDateToLevelMap.get(modelDate);
						count = count + 1;
					}
				}
				// store in a map here
				dateToLevelMap.put(model_date, value);
				villDataCountForDate.put(model_date, count);

			}
			// System.out.println("\n\n\nTime lag2 Village Level (1044)=
			...
			System.out.println("\n\n\nTime lag2A Village Level (1062)= " + (System.currentTimeMillis() - t3) + "\n\n");

			resultMap.put(mandalName, dateToLevelMap);
		}
		System.out.println("\n\nTime lag : Map(1063) Mandal level time lag  t1=" + t1 + " C= "
				+ System.currentTimeMillis() + "===> " + (System.currentTimeMillis() - t1));
	...
	public Map<String, IExtendedRainfallForecastData> getMultiSourcedRainfallForecast(int modelDate,
		 ...
		 * Use rf forecast value from {@link IWeatherForecast} if available for
		 * a rfLocUUID and rfForecastDate
	 ...
	 * It also increments the corresponding count of each field in the input
	 * Integer array.
	 * 
	 ...
	 *            - An Integer array whose indexes holds
	 *            the 'child-data count' for the attributes of
	 *            {@link ISoilMoistureStressData}
	 */
	 ...
	 * over
	 * grid's residualSM and availableSMinTMC, respectively. But for the
	 * District and
