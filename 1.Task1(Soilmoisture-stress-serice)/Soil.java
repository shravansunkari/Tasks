...
@Component
public class SoilMoistureStressService implements ISoilMoistureStressService {
	...
	public Map<String, ISoilMoistureStressData> getAllSMStressData(double rootZoneDepth)
		...
		int modelDate = nrscDataService
				.getLastKnownModelDateForNRSCData(DateUtils.getModelDateFromTs(System.currentTimeMillis()));
		if (modelDate == 0) {
			if (IS_DEBUG_ENABLED) {
		...
		if (rootZoneDepth < 0) {
			...
			throw new ObjectNotFoundException(
					"Unable to compute soil moisture " + "stress for a negative value of root depth");
		}

		/*
		 * System.out.
		 * println("ZBXXX : SMStressService - getAllSMStressData, Model Date = "
		 * +modelDate + ", Root-Zone Depth = "+rootZoneDepth);
		 */
	...
	public Map<String, ISoilMoistureStressData> getAllLastKnownSMStressData(int modelDate, double rootZoneDepth)
		...
		if (rootZoneDepth < 0) {
			if (IS_DEBUG_ENABLED) {
				System.out.println(
						"ZBXXX : SMStressService - getAllLastKnownSMStressData, Unable to compute soil moisture "
								+ "stress for a negative value of root depth, " + rootZoneDepth);
			}
			/*
			 * TODO Throw Proper Exception
		...
		/*
		 * System.out.
		 * println("ZBXXX : SMStressService - getAllLastKnownSMStressData, Model Date = "
		 * +modelDate + ", Root-Zone Depth = "+rootZoneDepth);
		 */
		return getAllSMStressData(modelDate, rootZoneDepth);
	}
	...
	public Map<Long, ISoilMoistureStressData> getAllGridSMStressData(int modelDate, double rootZoneDepth)
		...
		Map<String, ISoilMoistureStressData> villageSMStressMap = soilMoistureStressDataDSP
				.getSMStressDataAllVillages(modelDate, rootZoneDepth);
		...
		for (Entry<String, ISoilMoistureStressData> villageEntry : villageSMStressMap.entrySet()) {
			...
			if (!gridVillageMap.containsKey(villageName)) {
				/*
				 * System.out.
				 * println("ZBXXX : SMStressService - getAllGridSMStressData, "
				 * + "No Grid-Village map record found for village : " +
				 * villageName+", rootDepth : " + rootZoneDepth);
				 */
				continue;
			}
	...
	@Override
	public Map<Long, Double> getAllGridRFStressData(int modelDate) throws DSPException, ObjectNotFoundException {
		...
		for (ISoilMoistureNRSCGridData smNRSCGridData : smNRSCGridDataList) {
			...
			if (villList.size() != 0) {
				...
				for (ISMLocMapData vill : villList) {
					...
					if (!villageRFStressMap.containsKey(VillName)) {
						/*
						 * System.out.
						 * println("ZBXXX : SMStressService - getAllGridRFStressData, "
						 * + "No Grid Village map record found for village : " +
						 * villageName);
						 */
					} else {
			...
			/*
			 * else{ System.out.println("No Villages for grid"+smNRSCGridData.
			 * getGridId()); }
			 */
		}
	...
	public Map<String, ISoilMoistureStressData> getAllSMStressData(int modelDate, double rootZoneDepth)
		 ...
		 * 2. Get all village level computed data and also add it to the result
		 * map 3. Get MandalFN to villageFullName Mapping 4. For each district
		 * Iterate over each Mandal under the district [and compute district
		 * level average] Iterate over each village under the Mandal [and
		 * compute Mandal level average]
		 * 
		 * Note : Perform average for { availableSMPercentage,
		 * availableSMPer150cm, rootZoneAvailableSMPer, rootZoneAvailableSMInMM
		...
		Map<String, ISoilMoistureStressData> villageRFStressMap = soilMoistureStressDataDSP
				.getSMStressDataAllVillages(modelDate, rootZoneDepth);
		...
		for (String districtName : allDistToMandMap.keySet()) {
			...
			/*
			 * To store the total number of Mandal locations for which data is
			 * available, This total will be used to compute the average by
			 * dividing with the summed up values
			 * 
			 */
			...
			for (String mandalName : mandalList) {
				...
				/*
				 * To store the total number of village locations for which data
				 * is available, This total will be used to compute the average
				 * by dividing with the summed up values
				 * 
				 */
				...
				/*
				 * Compute average by dividing mandal level sum with villages
				 * count
				 */
	...
	@Override
	public Map<String, ISMRainfallStressData> getAllRFStressData(int modelDate)
			throws DSPException, ObjectNotFoundException {
		/*
		 * 1. Get the list of all SM-stress 'district names to Mandal names' map
		 * 2. Get all village level computed data and also add it to the result
		...
		for (String districtName : allDistToMandMap.keySet()) {
			...
			for (String mandalName : mandalList) {
				...
				/*
				 * Compute average by dividing mandal level sum with village
				 * count
				 */
	...
	protected Map<String, ISMRainfallStressData> getRFStressDataForVillages(int modelDate)
		...
		/*
		 * If modelDate = today's date then Apply rainfall Logic based on time
		 * else modelDate timeStamp must be greater than 8 : 30 i.e rainfall day
		 * start time
		 */
		...
		if (todayStartTs <= modelDateMillis && modelDateMillis <= todayEndTs) {
			...
			/*
			 * If the current TS is between 8:30:01 AM to 9:29:59 AM change it
			 * to 8:30 AM http://inkriti.net/jira/browse/VAS-247
			 */
			 ...
			 *
			 * NOTE : (3600 seconds/hour * 8 1/2 Hours) *1000 = 30600000
			 * milliseconds
			 *
			 */
		...
		long last3DaysTime = DateUtils.getIntervalOfDay(currDayStartTs - (3 * DateUtils.TWENTY_FOUR_HOURS_IN_SECONDS),
				8, 30, 0);

		/* Prepare a list of all rainfall location UUIDs */
		Map<String, List<String>> stateToRainfallLocMap = locationHierarchyService
				.getAllLocForParentChildTypes(LocationConstants.STATE, LocationConstants.RAINFALL);
		List<String> rainfallLocations = stateToRainfallLocMap.get(
				"6f86292b-dd9a-4987-bb8f-c3940263b349"); /*
															 * Andhra Pradesh
															 * LocationUUID
															 */
		...
		Map<String, IWaterQuantityEventData> last3DaysData = iwmService.getRFWQDataForStress(last3DaysTime,
				yesterdayEndTs, setOfLocations);
		Map<String, IWaterQuantityEventData> last7DaysData = iwmService.getRFWQDataForStress(last7DaysTime,
				yesterdayEndTs, setOfLocations);
		Map<String, IWaterQuantityEventData> last10DaysData = iwmService.getRFWQDataForStress(last10DaysTime,
				yesterdayEndTs, setOfLocations);
		Map<String, IWaterQuantityEventData> yesterdayData = iwmService.getRFWQDataForStress(yesterdayStartTs,
				yesterdayEndTs, setOfLocations);
		Map<String, IWaterQuantityEventData> todayData = iwmService.getRFWQDataForStress(currDayStartTs, currDayEndTs,
				setOfLocations);
		Map<String, IExtendedRainfallForecastData> rainfallForecastData = getMultiSourcedRainfallForecast(modelDate,
				rainfallLocations);

		Map<String, ISoilMoistureRFObject> nonRainyDaysData = iwmService.getVillageContinousNonRainyDays(currDayEndTs,
				IWMConstants.MIN_RAINFALL_LEVEL);

		/*
		 * Step 1 : Iterate over each village Step 2 : iterate over the list of
		 * rainfall station UUIDs under that village and compute average over
		 * all the stations under the village to get the village level data
		 * 
		 */
		for (String villageFullName : soilMoistLocDataMap.keySet()) {
			...
			/*
			 * To store the total count of locations for which data is
			 * available, This total will be used to compute the average by
			 * dividing with the summed up values
			 * 
			 */
			...
			for (ISMLocMapData smLocData : smLocDataList) {
				...
				if (last7DaysData.containsKey(rainfallUUID) && last7DaysData.get(rainfallUUID).getLevelValue1() >= 0) {
					villRFStress.setLast7DaysCumulative(
							villRFStress.getLast7DaysCumulative() + last7DaysData.get(rainfallUUID).getLevelValue1());
				...
				if (last3DaysData.containsKey(rainfallUUID) && last3DaysData.get(rainfallUUID).getLevelValue1() >= 0) {
					villRFStress.setLast3DaysCumulative(
							villRFStress.getLast3DaysCumulative() + last3DaysData.get(rainfallUUID).getLevelValue1());
				...
				if (todayData.containsKey(rainfallUUID) && todayData.get(rainfallUUID).getLevelValue1() >= 0) {
					villRFStress.setTodayActual(
							villRFStress.getTodayActual() + todayData.get(rainfallUUID).getLevelValue1());
				...
				if (yesterdayData.containsKey(rainfallUUID) && yesterdayData.get(rainfallUUID).getLevelValue1() >= 0) {
					villRFStress.setYesterdayActual(
							villRFStress.getYesterdayActual() + yesterdayData.get(rainfallUUID).getLevelValue1());
				...
				if (rainfallForecastData.containsKey(rainfallUUID)) {
					...
					/*
					 * For today's forecast, we use current rainfall, from
					 * tomorrow, we use forecasted data
					 */
					if (todayData.containsKey(rainfallUUID) && todayData.get(rainfallUUID).getLevelValue1() >= 0) {
						next7DaysForecastCum += todayData.get(rainfallUUID).getLevelValue1();
						villRFStress.setNext24HrForecast(
								villRFStress.getNext24HrForecast() + todayData.get(rainfallUUID).getLevelValue1());
					...
					if (isForecastDataAvailable) {
						villRFStress.setNext7DaysForecastCumulative(
								villRFStress.getNext7DaysForecastCumulative() + next7DaysForecastCum);
						villToStationCount[IWMConstants.SM_RF_7DAYS_FORECAST_INDEX]++;
					}
				}

				if (nonRainyDaysData.containsKey(rainfallUUID)
						&& nonRainyDaysData.get(rainfallUUID).getNoOfNonRainDays() >= 0) {
					villRFStress.setNoOfNonRainyDays(villRFStress.getNoOfNonRainyDays()
							+ nonRainyDaysData.get(rainfallUUID).getNoOfNonRainDays());
					villToStationCount[IWMConstants.SM_RF_NON_RAINY_DAYS_INDEX]++;
				}

				if (last10DaysData.containsKey(rainfallUUID)
						&& last10DaysData.get(rainfallUUID).getLevelValue1() >= 0) {
					villRFStress.setLast10DaysCumulative(
							villRFStress.getLast10DaysCumulative() + last10DaysData.get(rainfallUUID).getLevelValue1());
					villToStationCount[IWMConstants.SM_RF_LAST10DAYS_INDEX]++;
				}

			}
			/*
			 * Compute average by dividing village level sum with rf-stations
			 * count
			 */
	...
	@Override
	public Map<String, Map<Integer, ISoilMoistureStressData>> getSMStressDataForRange(int startDate, int endDate,
			List<String> locationFullNameList, double rootZoneDepth) throws DSPException, ObjectNotFoundException {
		...
		/*
		 * Distinguish mandal names and village names, store each of them in a
		 * separate List
		 */
		...
		/*
		 * Make a single list of all villageNames i. By adding all the given
		 * villages to it ii. By adding all the villages under the given Mandals
		 * 
		 */
		...
		System.out.println("\n\nTime lag : Map(712) time lag  t1=" + t1 + " C= " + System.currentTimeMillis() + "===> "
		...
		for (String mandalName : mandalVillMap.keySet()) {
			...
			for (String villageName : villageList) {
				...
				if (villDateToDataMap == null) {
					if (IS_DEBUG_ENABLED) {
						System.out.println(
								"ZBXXX : SMStressService - getSMStressDataForRange, No SM Stress data available "
										+ "for village = " + villageName + ", mandal = " + mandalName + ", between "
				...
				for (Integer modelDate : villDateToDataMap.keySet()) {
					...
					if (mandStressData == null) {
						...
						/*
						 * To store the total count of grids for which data is
						 * available, This total will be used to compute the
						 * average by dividing with the summed up values
						 * 
						 */
		...
		System.out.println("\n\nTime lag : Map(793) time lag  t1=" + t1 + " C= " + System.currentTimeMillis() + "===> "
				+ (System.currentTimeMillis() - t1));

		/*
		 * Remove the entries for the 'extra-villages' which are under the given
		 * Mandals (if any)
		 */
		t1 = System.currentTimeMillis();
		for (String mandalName : mandalVillMap.keySet()) {
		...
		System.out.println("\n\nTime lag : Map(809) time lag  t1=" + t1 + " C= " + System.currentTimeMillis() + "===> "
				+ (System.currentTimeMillis() - t1));
		return resultMap;
	}

	@Override
	public Map<String, Map<Integer, Double>> getSMStressRFDataForDate(int startDate, int endDate,
			List<String> locationFullNameList) throws DSPException, ObjectNotFoundException {
		...
		/*
		 * Make a single list of all the rainfall locationUUIDs, associated with
		 * the given village locations
		 */
		 ...
		 */
		Map<String, List<IWaterQuantityEventData>> rfData = iwmService.getWaterQuantityEventsInTimeRange(allRFUUIDList,
				startTs, endTs, SourceTypeConstants.RAINFALL);

		/*
		 * 
		 * 1. For each of the given village 2. Create a map, M1, to store 'date'
		 * to 'dataRecordsCount' 3. Create a map, M2, to store 'date' to
		 * 'rainfallLevelData' 4. For every grid under the village, find out
		 * it's rfStation 5. For every data record associated with that
		 * rfStation 6. Find out it's date and compute sum with the existing
		 * data in M2 for that date and store it back in M2 7. Increment the
		 * dataRecordsCount for that date in M1 8. For each (date, Data) pair in
		 * M2 9. compute average by dividing the Data with the
		 * 'dataRecordsCount' avilable from M1 for that date.
		 * 
		 */
		for (String villageName : villageSMLocDataMap.keySet()) {

			/*
			 * i. Getting list of all gridIDs for the given village ii. Creating
			 * a map to store date to Data iii. Creating a map to store date to
			 * dataCount
			 * 
			 */
			List<ISMLocMapData> gridsUnderVillage = villageSMLocDataMap.get(villageName);
			...
			/*
			 * Use temporary map<DateModel, id> to map model dates to id's so
			 * that they can be stored in an array which leads to less run time.
			 * 
			 */
			// dateId is to assign unique id to each model date
			int dateId = 1;

			// Get total number of dates from startDate to endDate
			int noOfDays = DateUtils.substractModelDates(startDate, endDate);

			// Get list of all dates in the format yyyyMMdd between startDate
			// and endDate
			List<Integer> datesList = DateUtils.addNDaysFromModelDate(startDate, noOfDays);

			// map each model date to a unique id refered by dateId
			Map<Integer, Integer> dateMap = new HashMap<>();

			// Store values and counts in temporary arrays
			double[] values_arr = new double[366];
			int[] counts_arr = new int[366];
			Arrays.fill(values_arr, 0.0);
			Arrays.fill(counts_arr, 0);

			// Assign id for each model date and store in HashMap
			for (Integer str : datesList) {
				dateMap.put(str, dateId);
				dateId = dateId + 1;
			}
			...
			for (ISMLocMapData gridData : gridsUnderVillage) {
				...
				if (rfLocDataList == null) {
					if (IS_DEBUG_ENABLED) {
						System.out.println(
								"ZBXXX : SMStressService - getSMStressRFDataForDate, No rainfall data available "
										+ "for grid = " + gridData.getGridId() + " with rfLocUUID = " + rfLocationUUID);
					}
					continue;
				}
				/*
				 * Sum all the station grid values in a variable Store the count
				 * of stations in a variable
				 */
				...
				for (IWaterQuantityEventData wqEventData : rfLocDataList) {
					...
					if (wqEventData.getLevelValue1() >= 0) {
						int id = dateMap.get(modelDate);
						values_arr[id] += wqEventData.getLevelValue1();
						counts_arr[id] += 1;
						if (!dataExist) {
							dataExist = true;
						}
			...
			for (Integer modelDate1 : dateToLevelMap.keySet()) {
				int id = dateMap.get(modelDate1);
				if (counts_arr[id] != 0) {
					values_arr[id] = values_arr[id] / counts_arr[id];
				} else {
					values_arr[id] = DSPConstants.NO_DATA;
				}
			}

			for (Integer modelDate1 : dateToLevelMap.keySet()) {
				int id = dateMap.get(modelDate1);
				dateToLevelMap.put(modelDate1, values_arr[id]);
			}
		...
		for (String mandalName : mandalVillMap.keySet()) {
			/*
			 * i. Getting list of all villages for the given Mandal ii. Creating
			 * a map to store date to Data iii. Creating a map to store date to
			 * dataCount
			 * 
			 */
			...
			// map each model date to a unique id referred by dateId
			Map<Integer, Integer> dateMap = new HashMap<>();
			...
			for (String villageName : villageList) {
				...
				for (Integer modelDate : villDateToLevelMap.keySet()) {
					if (villDateToLevelMap.get(modelDate) >= 0) {
						int id = dateMap.get(modelDate);
						values_arr[id] += villDateToLevelMap.get(modelDate);
						counts_arr[id] += 1;
						dataExist = true;
					}
				}
	...
	public Map<String, Map<Integer, ISoilMoistureStressData>> getSMVillageStressData(int startDate, int endDate,
		...
		/*
		 * Make a list of all Monsoons that falls within the given startDate and
		 * endDate
		 * 
		 */
		...
		/*
		 * Get ISMLocMapData for all the given villages. And then make a list of
		 * all required gridIDs
		 * 
		 */
		...
		/*
		 * DBCall - Get the GridData for all the required gridIDs And then make
		 * a Map from GridID to GridData
		 */
		 ...
		 */
		Map<Long, List<ISoilMoistureNRSCData>> nrscDataMap = nrscDataService.getNRSCData(startDate, endDate,
				gridIDList);
		 ...
		 * 1. For each of the given village 2. Create a map, M1, to store 'date'
		 * to 'dataRecordsCount' 3. Create a map, M2, to store 'date' to
		 * 'actualData' 4. For every grid under the village 5. For every data
		 * record associated with that grid 6. Find out it's date and compute
		 * sum with the existing data in M2 for that date and store it back in
		 * M2 7. Increment the dataRecordsCount for that date in M1 8. For each
		 * (date, Data) pair in M2 9. compute average by dividing the Data with
		 * the 'dataRecordsCount' avilable from M1 for that date.
		 * 
		 */
		for (String villageFullName : villageList) {
			...
			for (ISMLocMapData smLocData : smLocDataList) {
				...
				/*
				 * Get the following data for the grid : i. NRSC grid data ii.
				 * All NRSC soil moisture data between the given dates
				 */
				...
				/*
				 * Iterate over each of the fetched NRSC soil moisture record
				 * for the grid
				 */
				for (ISoilMoistureNRSCData nrscData : nrscCurDataList) {
					...
					gridStressData.setSoilMoisturePercent(soilMoistureStressDataDSP
							.computeAvailableSMPercentage(nrscData, gridData, IWMConstants.SM_DEFAULT_TOTAL_DEPTH));
					gridStressData.setRootZoneAvailableSoilMoistureInMM(
							soilMoistureStressDataDSP.computeAvailableSMInMM(nrscData, gridData, rootZoneDepth));
					gridStressData.setAverageSMRootZone(
							soilMoistureStressDataDSP.computeAvailableSMPercentage(nrscData, gridData, rootZoneDepth));
					Double june1stSMContent = soilMoistureStressDataDSP.computeMoistureContent(june1stData, gridData,
							smLocData, rootZoneDepth);
					Double currSMContent = soilMoistureStressDataDSP.computeMoistureContent(nrscData, gridData,
							smLocData, rootZoneDepth);
					...
					addSoilMoistureStressDataForVillage(gridStressData, villageStressData, villToGridCount,
							currSMContent, june1stSMContent);
				}
			}
	...
	public Map<String, IExtendedRainfallForecastData> getMultiSourcedRainfallForecast(int modelDate,
		...
		/*
		 * 1. Gets all weatherForecastData at rainfall station level from ISRO.
		 * 2. Gets {@link IRainfallForecast} data from APSDPS 3. Prepare {@link
		 * ICSRFForecastData} for crop stress computation Use rf forecast value
		...
		/*
		 * 1. Get rainfall forecast data from ISRO/WeatherForecast service 2.
		 * Get rainfall forecast data from APSDPS
		 */
	...
	/**
	 * This method adds the fields of child {@link ISoilMoistureStressData} to
	 * the corresponding parent {@link ISoilMoistureStressData} fields. It also
	 * increments the corresponding count of each field in the input Integer
	 * array.
	 * 
	 * NOTE : This method is used only if the parent-level is either Mandal or
	 * District
	 * 
	 * @param childStressData
	 *            {@link ISoilMoistureStressData}
	 * @param parentStressData
	 *            {@link ISoilMoistureStressData}
	 * @param parentToChildCount
	 *            - An Integer array whose indexes holds the 'child-data count'
	 *            for the attributes of {@link ISoilMoistureStressData}
	 */
	protected void addSoilMoistureStressData(ISoilMoistureStressData childStressData,
		...
		/*
		 * Store the list of Soil Moisture percent of the Villages (used in UI
		 * for filtering) at both Mandal and District levels
		 */
		...
		if (rootZoneAvailableSMInMM >= 0) {
			parentStressData.setRootZoneAvailableSoilMoistureInMM(
					parentStressData.getRootZoneAvailableSoilMoistureInMM() + rootZoneAvailableSMInMM);
		...
		if (availableSMInTMC >= 0) {
			parentStressData
					.setAvailableSoilMoistureInTMC(parentStressData.getAvailableSoilMoistureInTMC() + availableSMInTMC);
			parentToChildCount[IWMConstants.SM_AVLBL_SM_IN_TMC_INDEX]++;
		}
	}

	/**
	 * This method computes averages of the {@link ISoilMoistureStressData}
	 * fields by dividing them with the corresponding count of each field from
	 * the input Integer array.<br>
	 * <br>
	 * 
	 * <b>NOTE : </b><br>
	 * At village level residualSM and availableSMinTMC are computed as average
	 * over grid's residualSM and availableSMinTMC, respectively. But for the
	 * District and Mandal Levels, they should be just summed up.
	 * 
	...
	protected void computeAverageSMStressData(ISoilMoistureStressData parentStressData, Integer[] parentToChildCount) {
		/*
		 * If data is not available, then use IWMConstants.NO_DATA to explicitly
		 * indicate 'No Data' condition
		 */

		if (parentToChildCount[IWMConstants.SM_AVLBL_SM_PERCENT_INDEX] > 0) {
			parentStressData.setSoilMoisturePercent(parentStressData.getSoilMoisturePercent()
					/ parentToChildCount[IWMConstants.SM_AVLBL_SM_PERCENT_INDEX]);
		} else {
		...
		if (parentToChildCount[IWMConstants.SM_AVLBL_SM_PERCENT_150CM_INDEX] > 0) {
			parentStressData.setAverageSM15m(parentStressData.getAverageSM15m()
					/ parentToChildCount[IWMConstants.SM_AVLBL_SM_PERCENT_150CM_INDEX]);
		} else {
		...
		if (parentToChildCount[IWMConstants.SM_AVLBL_SM_PERCENT_ROOTZONE_INDEX] > 0) {
			parentStressData.setAverageSMRootZone((parentStressData.getAverageSMRootZone()
					/ parentToChildCount[IWMConstants.SM_AVLBL_SM_PERCENT_ROOTZONE_INDEX]));
		} else {
		...
		if (parentToChildCount[IWMConstants.SM_AVLBL_SM_ROOTZONE_IN_MM_INDEX] > 0) {
			parentStressData
					.setRootZoneAvailableSoilMoistureInMM(parentStressData.getRootZoneAvailableSoilMoistureInMM()
							/ parentToChildCount[IWMConstants.SM_AVLBL_SM_ROOTZONE_IN_MM_INDEX]);
		} else {
	 ...
	 *            for the attributes of {@link ISoilMoistureStressData}
	 */
	protected void computeAverageSMStressDataForVillage(ISoilMoistureStressData parentStressData,
			Integer[] parentToChildCount) {

		computeAverageSMStressData(parentStressData, parentToChildCount);

		if (parentToChildCount[IWMConstants.SM_RESIDUAL_SM_INDEX] > 0) {
			parentStressData.setResidualSM(
					parentStressData.getResidualSM() / parentToChildCount[IWMConstants.SM_RESIDUAL_SM_INDEX]);
		}

		if (parentToChildCount[IWMConstants.SM_AVLBL_SM_IN_TMC_INDEX] > 0) {
			parentStressData.setAvailableSoilMoistureInTMC(parentStressData.getAvailableSoilMoistureInTMC()
					/ parentToChildCount[IWMConstants.SM_AVLBL_SM_IN_TMC_INDEX]);
		}
	}
	 ...
	 *            for the attributes of {@link ISoilMoistureStressData}
	 * @param currSMContent
	 *            - A double value representing the current soil moisture
	 *            content in TMC
	 * @param june1stSMContent
	 *            - A double value representing the June1st soil moisture
	 *            content in TMC
	 */
	protected void addSoilMoistureStressDataForVillage(ISoilMoistureStressData childStressData,
			ISoilMoistureStressData parentStressData, Integer[] parentToChildCount, double currSMContent,
			double june1stSMContent) {
	 ...
	 * by dividing them with the corresponding count of each field from the
	 * input Integer array.<br>
	 * <br>
	 * 
	 * @param parentStressData
	 *            {@link ISMRainfallStressData}
	 * @param parentToChildCount
	 *            - An Integer array whose indexes holds the 'child-data count'
	 *            for the attributes of {@link ISMRainfallStressData}
	 */
	protected void computeAverageRainfallStressData(ISMRainfallStressData parentStressData,
			Integer[] parentToChildCount) {
		/*
		 * If data is not available, then use IWMConstants.NO_DATA to explicitly
		 * indicate 'No Data' condition
		...
		if (parentToChildCount[IWMConstants.SM_RF_LAST7DAYS_INDEX] > 0) {
			parentStressData.setLast7DaysCumulative(
					parentStressData.getLast7DaysCumulative() / parentToChildCount[IWMConstants.SM_RF_LAST7DAYS_INDEX]);
		} else {
			parentStressData.setLast7DaysCumulative(DSPConstants.NO_DATA);
		}

		if (parentToChildCount[IWMConstants.SM_RF_LAST3DAYS_INDEX] > 0) {
			parentStressData.setLast3DaysCumulative(
					parentStressData.getLast3DaysCumulative() / parentToChildCount[IWMConstants.SM_RF_LAST3DAYS_INDEX]);
		} else {
			parentStressData.setLast3DaysCumulative(DSPConstants.NO_DATA);
		}

		if (parentToChildCount[IWMConstants.SM_RF_TODAY_INDEX] > 0) {
			parentStressData.setTodayActual(
					parentStressData.getTodayActual() / parentToChildCount[IWMConstants.SM_RF_TODAY_INDEX]);
		} else {
			parentStressData.setTodayActual(DSPConstants.NO_DATA);
		}

		if (parentToChildCount[IWMConstants.SM_RF_YESTERDAY_INDEX] > 0) {
			parentStressData.setYesterdayActual(
					parentStressData.getYesterdayActual() / parentToChildCount[IWMConstants.SM_RF_YESTERDAY_INDEX]);
		} else {
			parentStressData.setYesterdayActual(DSPConstants.NO_DATA);
		}

		if (parentToChildCount[IWMConstants.SM_RF_24HRS_FORECAST_INDEX] > 0) {
			parentStressData.setNext24HrForecast(parentStressData.getNext24HrForecast()
		...
		if (parentToChildCount[IWMConstants.SM_RF_48HRS_FORECAST_INDEX] > 0) {
			parentStressData.setNext48HrForecast(parentStressData.getNext48HrForecast()
		...
		if (parentToChildCount[IWMConstants.SM_RF_NON_RAINY_DAYS_INDEX] > 0) {
			parentStressData.setNoOfNonRainyDays(Math.round(((float) parentStressData.getNoOfNonRainyDays())
		...
		if (parentToChildCount[IWMConstants.SM_RF_LAST10DAYS_INDEX] > 0) {
			parentStressData.setLast10DaysCumulative(parentStressData.getLast10DaysCumulative()
		...
		if (parentToChildCount[IWMConstants.SM_RF_7DAYS_FORECAST_INDEX] > 0) {
			parentStressData.setNext7DaysForecastCumulative(parentStressData.getNext7DaysForecastCumulative()
		 ...
		 * 
		 * Since we have computed final rainfall data value for today and
		 * yesterday, we can simply add them up.
		 */
		...
		if (parentStressData.getYesterdayActual() >= 0) {
			parentStressData.setRainfallSinceYesterday(
					parentStressData.getRainfallSinceYesterday() + parentStressData.getYesterdayActual());
			rfDataPresentSinceYesterday = true;
		}
	...
	/**
	 * 
	 * This method adds the fields of child {@link ISMRainfallStressData} to the
	 * corresponding parent {@link ISMRainfallStressData} fields. It also
	 * increments the corresponding count of each field in the input Integer
	 * array.
	 * 
	 * @param childStressData
	...
	protected void addRainfallStressData(ISMRainfallStressData childStressData, ISMRainfallStressData parentStressData,
		...
		if (childStressData.getLast7DaysCumulative() >= 0) {
			parentStressData.setLast7DaysCumulative(
					parentStressData.getLast7DaysCumulative() + childStressData.getLast7DaysCumulative());
			parentToChildCount[IWMConstants.SM_RF_LAST7DAYS_INDEX]++;
		}
		if (childStressData.getLast3DaysCumulative() >= 0) {
			parentStressData.setLast3DaysCumulative(
					parentStressData.getLast3DaysCumulative() + childStressData.getLast3DaysCumulative());
		...
		if (childStressData.getYesterdayActual() >= 0) {
			parentStressData
					.setYesterdayActual(parentStressData.getYesterdayActual() + childStressData.getYesterdayActual());
			parentToChildCount[IWMConstants.SM_RF_YESTERDAY_INDEX]++;
		}
		if (childStressData.getNext24HrForecast() >= 0) {
			parentStressData.setNext24HrForecast(
					parentStressData.getNext24HrForecast() + childStressData.getNext24HrForecast());
			parentToChildCount[IWMConstants.SM_RF_24HRS_FORECAST_INDEX]++;
		}
		if (childStressData.getNext48HrForecast() >= 0) {
			parentStressData.setNext48HrForecast(
					parentStressData.getNext48HrForecast() + childStressData.getNext48HrForecast());
			parentToChildCount[IWMConstants.SM_RF_48HRS_FORECAST_INDEX]++;
		}
		if (childStressData.getNoOfNonRainyDays() >= 0) {
			parentStressData.setNoOfNonRainyDays(
					parentStressData.getNoOfNonRainyDays() + childStressData.getNoOfNonRainyDays());
			parentToChildCount[IWMConstants.SM_RF_NON_RAINY_DAYS_INDEX]++;
		}
		if (childStressData.getLast10DaysCumulative() >= 0) {
			parentStressData.setLast10DaysCumulative(
					parentStressData.getLast10DaysCumulative() + childStressData.getLast10DaysCumulative());
			parentToChildCount[IWMConstants.SM_RF_LAST10DAYS_INDEX]++;
		}
		if (childStressData.getNext7DaysForecastCumulative() >= 0) {
			parentStressData.setNext7DaysForecastCumulative(parentStressData.getNext7DaysForecastCumulative()
					+ childStressData.getNext7DaysForecastCumulative());
			parentToChildCount[IWMConstants.SM_RF_7DAYS_FORECAST_INDEX]++;
		}
	}
